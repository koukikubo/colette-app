...
table_type_master_key = "restaurant_table_type"
...
ActiveRecord::Base.transaction do
  (counter_seats + table_seats).each do |seat|
    table_type = seat.fetch(:restaurant_table_type)
    name = seat.fetch(:name)

    next if RestaurantTable.exists?(
      restaurant_table_type: table_type,
      name: name
    )
...
puts "登録件数: #{RestaurantTable.count}件"
puts "総席数: #{RestaurantTable.sum(:capacity)}席"